...
const extract = require('./web/extract').main
...
exports.getSongsJson = ($, url, quality) => {
  // 在 playlist 页面有一个 textarea 标签
  // 包含playlist 的详细信息
  // const text = $('ul.f-hide')
  //   .next('textarea')
  //   .html()
  //
  // 2018-01-19 更新
  // 需要调用 web/extract 解析出来

  const textarea = $('#song-list-pre-cache textarea')
    .eq(0)
    .val()

  // D5I = decodeURIComponent(k5p.cAF7y(D5I, "param=" + boW9N + VO3x))
  const minfoId = $('#m-playlist .m-info').attr('id')
  const paramX = minfoId.slice(8, 12)
  const paramY = $('#m-playlist .j-img').attr('data-key')

  debug(
    'playlist info: textarea.length = %s, minfoId = %s, param=%s%s',
    textarea.length,
    minfoId,
    paramX,
    paramY
  ...
  let text = extract(textarea, 'param=' + paramX + paramY)
  text = decodeURIComponent(text)
  let songs = JSON.parse(text)
  debug('extract songs = %j', songs)
  return songs
}
...
exports.getSongs = async function($, url, quality) {
  ...
  // 基本信息
  let songs = exports.getSongsJson($, url, quality)
