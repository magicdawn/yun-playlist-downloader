...
exports.downloadSong = async function(url, file, song, totalLength, timeout, maxTimes, skipExists) {
  let bar
  const initBar = () => {
    bar = new ProgressBar({
  ...
  // 下载中
  let downloading = percent => bar.update(percent, { symbol: symbols.info, postText: file })

  // 成功
  let success = () => bar.update(1, { symbol: symbols.success, postText: file })

  // 失败
  let fail = () => bar.update(0, { symbol: symbols.error, postText: file })

  // 重试
  let retry = i => {
    bar.update(0, { symbol: symbols.warning, postText: `${i + 1}次失败` })
    // reinit
    initBar()
  }
