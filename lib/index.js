...
const debug = require('debug')('yun:lib:index');
...
exports.tryDownloadSong = co.wrap(function*(url, filename, song, totalLength, timeout, maxTimes, skipExists) {
  // 不用下载
  const needDownload = yield exports.needDownload(url, filename, skipExists);
  if (!needDownload) {
    console.log(`${ symbols.success } ${ song.index }/${ totalLength } 下载跳过 ${ filename }`);
    return;
  }
  ...
  const tryDownload = pretry(exports.download, {
    ...
    onerror: function(e, i) {
      console.log(`${ symbols.warning } ${ song.index }/${ totalLength }  ${i + 1}次失败 ${ filename }`);
    }
  });
